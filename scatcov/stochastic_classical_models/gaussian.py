...
def gaussian_cme(cov, R, T):
    ...
    if np.any(L.real < 0):
        warnings.warn(
            'Found FFT of covariance < 0. Embedding matrix is not non-negative definite.'
        )
...
def fbm(R, T, H, sigma=1, dt=None):
    ...
    n = np.arange(T)
    r = dt**(2 * H) * sigma**2 / 2 * (np.abs(n + 1)**(2 * H) + np.abs(n - 1)**
                                      (2 * H) - 2 * np.abs(n)**(2 * H))
...
def geom_brownian(R: int, T: float, nb_sample: int, S0: float, mu: float,
                  sigma: float):
    ...
    X = S0 * np.exp((mu - 0.5 * sigma**2) * np.arange(T) + sigma * np.sqrt(T) *
                    (B - B[:, 0:1]))
