...
from scatcov.global_const import *
...
class ProcessDataLoader:
    ...
    def __init__(self,
        ...
        self.dir_name = Path(
            __file__).parents[0] / 'cached_dir' if dirname is None else Path(
                dirname)
    ...
    def dirpath(self, **kwargs) -> Path:
        ...
        fname = (self.model_name + format_path(None, kwargs)).replace(
            '.', '_').replace('-', '_')
    ...
    def generate(self, dirpath: Path, S_gen: int, **kwargs) -> dict:
        ...
        kwargs_gen = {
            key: value
            for key, value in kwargs.items() if key != 'B'
        }
    ...
    def load(self, **kwargs) -> TimeSeriesDir:
        ...
        dirpath = self.dirpath(
            **
            {key: value
             for (key, value) in full_kwargs.items() if key != 'B'})
        ...
        if R_available < full_kwargs['B']:
            ...
            self.generate(S_gen=full_kwargs['B'] - R_available,
                          dirpath=dirpath,
                          **full_kwargs)
        if len(list(dirpath.glob('*'))) < full_kwargs['B']:
            print(
                f"Incomplete generation {len(list(dirpath.glob('*')))}/{full_kwargs['B']}."
            )
...
class PoissonLoader(ProcessDataLoader):
    ...
    def generate_trajectory(self, **kwargs):
        return poisson_mu(T=kwargs['T'],
                          mu=kwargs['mu'],
                          signed=kwargs['signed'])[None, :]
