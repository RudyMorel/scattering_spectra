...
class MSELossScat(nn.Module):
    ...
    def compute_gap(self, input: Optional[DescribedTensor],
                    target: DescribedTensor, weights):
        if input is None:
        ...
        self.max_gap = {
            m_type:
            torch.max(cplx.modulus(
                gap[target.descri.where(m_type=m_type), :])).item()
...
class MSELossCov(nn.Module):
    ...
    def forward(self, input, target):
        ...
        # exp
        gap[target.descri.where(q=1), :] = cplx.mul(
            target.select(q=1),
            input.select(q=1) - target.select(q=1))

        # cov
        gap[target.descri.where(
            q=2), :] = input.select(q=2) - target.select(q=2)

        self.max_gap = {
            m_type: torch.max(
                cplx.modulus(gap[target.descri.where(m_type=m_type)])).item()
            for m_type in np.unique(target.descri['m_type'])
        }
