...
class Marginal(SubModuleChunk):
    ...
    def external_surjection_aux(self,
        ...
        output_descri_l = [
            namedtuple('Descri', out_columns)(*extend(input_descri, q))
            for q in self.qs
        ]
    ...
    def init_one_chunk(self, input: DescribedTensor,
        ...
        for q in self.qs:
            mask_q = multid_where_np(
                output_descri.reduce(q=q).drop(columns=['q', 'm_type']).values,
                input.descri.values)
            masks.append(mask_q)
    ...
    def forward_chunk(self, x: torch.Tensor, i_chunk: int) -> DescribedTensor:
        ...
        for q, mask_q in zip(self.qs, mask_qs):
            y[:, descri.where(q=q), 0,
              0] = cplx.modulus(x[:, mask_q, ...]).pow(q).mean(-1)
...
class Cov(SubModuleChunk):
    ...
    def __init__(self,
                 ...
                 m_types: Optional[List[str]] = None):
        super(Cov, self).__init__(init_with_input=False)
        self.N = N
        self.sc_idxer = sc_idxer

        possible_m_types = [
            f'm{r1}{r2}' for (r1, r2) in product(range(self.sc_idxer.r_max),
                                                 range(self.sc_idxer.r_max))
            if r1 >= r2
        ]
    ...
    def external_surjection_aux(self, row: NamedTuple) -> List[NamedTuple]:
        ...
        if 'm00' in self.m_types and low:
            output_descri_l.append(
                (n1, n1, 1, max(1, r - 1), -1, sc, -1, *js, *(-1, ) * len(js),
                 a, -1, low, r == 1, 'm00'))
        ...
        for scp in self.sc_idxer.get_all_idx():
            ...
            (scl,
             scr) = (sc,
                     scp) if r > rp or (r == rp and path >= path_p) else (scp,
                                                                          sc)
            jl, jr = self.sc_idxer.idx_to_path(
                scl, squeeze=False), self.sc_idxer.idx_to_path(scr,
                                                               squeeze=False)
            ...
            for n1p in range(n1, self.N):
                if (rr == rl == 1) and n1p < n1:
                    continue
                out_descri = (n1, n1p, 2, rl, rr, scl, scr, *jl, *jr, a, a, low
                              or scl == scr, low, f'm{rl-1}{rr-1}')
                output_descri_l.append(out_descri)

        return [
            namedtuple('Description', out_columns)(*row)
            for row in output_descri_l
        ]
    ...
    def init_one_chunk(self, input: DescribedTensor,
        ...
        # mask q = 1: E[SX]
        mask_q1 = multid_where_np(
            output_descri.reduce(q=1).to_array(['n1', 'sc']), channel_scale)

        # mask q = 2: E[SX SX^*]
        mask_q2_l = multid_where_np(
            output_descri.reduce(q=2).to_array(['n1', 'sc']), channel_scale)
        mask_q2_r = multid_where_np(
            output_descri.reduce(q=2).to_array(['n1p', 'scp']), channel_scale)
...
class CovStat(SubModuleChunk):
    ...
    def external_surjection_aux(self, row: NamedTuple) -> List[NamedTuple]:
        ...
        out_columns = [
            'n1', 'n1p', 'q', 'r', 'rp', 'j', 'a', 'b', 're', 'low', 'm_type'
        ]
        row_l = []
        ...
        if m_type == 'm11' and 'm11' in self.m_types:
            ...
            row_l.append((n1, n1p, q, r1, rp1, j1 if low else -1, a, b, a == 0
                          or low, low, 'm11'))
    ...
    def construct_proj(self, input: DescribedTensor,
                       output_descri: Description) -> torch.tensor:
        ...
        # init projector
        proj = torch.zeros(output_descri.size(),
                           input_descri.size(),
                           dtype=torch.float64)
    ...
    def init_one_chunk(self, input: DescribedTensor,
                       output_descri: Description, i_chunk: int) -> None:
        """ Init the parameters of the model required to compute output_descri from input. """
        self.register_buffer(f'proj_{i_chunk}',
                             self.construct_proj(input, output_descri))
