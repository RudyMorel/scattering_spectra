...
class ReflectionPad(Pad1d):
    ...
    def __init__(self, T):
        ...
        self.register_buffer('unpad_idx',
                             torch.tensor(unpad_idx, dtype=torch.int64))
...
class Wavelet(SubModuleChunk):
    ...
    def __init__(self, T, J, Q, wav_type, high_freq, wav_norm, layer_r,
                 sc_idxer):
        ...
        self.filt_hat = nn.Parameter(cplx.from_np(
            np.concatenate([psi_hat, phi_hat[None, :]])),
                                     requires_grad=False)
    ...
    def external_surjection_aux(self, input_descri):
        ...
        for jrp1 in range(0 if r == 0 else js[r - 1] + 1,
                          self.sc_idxer.JQ() + 1):
    ...
    def init_one_chunk(self, input: DescribedTensor,
        ...
        columns_previous_layer = ['n1'] + [
            f'j{r}' for r in range(1, self.layer_r)
        ]
        n_j1 = output_descri.to_array(columns_previous_layer)
        sc_pairing_left = multid_where_np(
            n_j1, input.descri.to_array(columns_previous_layer))
    ...
    def forward_chunk(self, x: torch.tensor, i_chunk: int):
        ...
        x_filt_hat = cplx.mul(x_hat[..., pairing[:, 0], :, :],
                              self.filt_hat[pairing[:, 1], :, :])
        x_filt = self.Pad.unpad(ifft1d_c2c_normed(x_filt_hat))

        return DescribedTensor(x=None,
                               descri=descri,
                               y=x_filt.reshape((x_filt.shape[0], -1) +
                                                x_filt.shape[-2:]))
...
class SpectrumNormalization(SubModuleChunk):
    ...
    def __init__(self,
                 on_the_fly: Optional[bool] = False,
                 sigma: Optional[DescribedTensor] = None):
        super(SpectrumNormalization, self).__init__()
        if not on_the_fly and sigma is None:
            raise ValueError(
                "SpectrumNormalization requires a power-spectrum to use as normalization."
            )
        self.on_the_fly = on_the_fly
        self.sigma_descri = sigma.descri
        self.register_buffer('sigma',
                             cplx.real(sigma.y).pow(0.5).unsqueeze(-1))
    ...
    def init_one_chunk(self, input: DescribedTensor,
        ...
        # mask q = 1: E[SX]
        mask = multid_where_np(output_descri.to_array(['n1', 'sc']),
                               channel_scale)
    ...
    def forward_chunk(self, x: torch.tensor, i_chunk: int):
        ...
        if self.on_the_fly:
            sigma = cplx.modulus(x).pow(2.0).mean(-1).pow(0.5).unsqueeze(
                -1).unsqueeze(-1)
        else:
