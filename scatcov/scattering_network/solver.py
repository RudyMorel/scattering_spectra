...
def compute_w_l2(weights, model, w_gap, nchunks):
    ...
    nbs = {
        m_type: model.count_coefficients(m_type=m_type)
        for m_type in model.m_types
    }

    weights_sum = sum(
        [weights[m_type] * nbs[m_type] for m_type in model.m_types])
    ...
    for i_chunk in range(nchunks):
        ...
        for m_type in model.m_types:
            coeff_type_mask = model.descri_chunked[-1][i_chunk].where(
                m_type=m_type)
...
class Solver(nn.Module):
    ...
    def __init__(self,
                 model: ModuleChunk,
                 loss: nn.Module,
                 xf: Optional[torch.Tensor] = None,
                 Rxf: Optional[DescribedTensor] = None,
                 x0: Optional[torch.Tensor] = None,
                 weights: Optional[Dict[str, float]] = None,
                 cuda: bool = False,
                 relative_optim: bool = False):
        ...
        if relative_optim:
            eps = 1e-7
            lRxf = [
                -self.loss.compute_gap(None, Rxf_chunk, None)
                for Rxf_chunk in self.Rxf
            ]
            self.w_gap = [(relu(cplx.modulus(lRxf_chunk) - eps) + eps)**(-1)
                          for lRxf_chunk in lRxf]
        ...
        for i_chunk in range(self.nchunks):
            Rx0_chunk = self.model(self.format(x0, requires_grad=False),
                                   i_chunk)
            self.loss0 += self.loss(Rx0_chunk, self.Rxf[i_chunk],
                                    self.w_gap[i_chunk],
                                    self.w_l2[i_chunk]).detach().cpu().numpy()
    ...
    def joint(self, x: np.ndarray):
        ...
        for i_chunk in range(self.nchunks):
            ...
            loss = self.loss(Rxt_chunk, self.Rxf[i_chunk], self.w_gap[i_chunk],
                             self.w_l2[i_chunk])
            res_max = {
                m_type: max(
                    res_max[m_type], self.loss.max_gap[m_type]
                    if m_type in self.loss.max_gap else 0.0)
                for m_type in self.model.module_list[-1].m_types
            }
    ...
    def print_info_line(self, msg: Optional[str] = ''):
        ...
        print(
            colored(
                "".join([
                    f"\n ----- {m_type} {value:.2e}, "
                    for m_type, value in self.max_gap.items()
                ]) + msg, 'green'))
