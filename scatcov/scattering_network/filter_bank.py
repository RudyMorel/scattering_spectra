...
def battle_lemarie_psi(T, Q, xi, normalize):
    ...
    stable_den[freqs != 0] = np.sqrt(
        den[freqs != 0]) / (2 * np.pi * freqs[freqs != 0])**4
    ...
    while True:
        idx += 1
        if idx == psi_hat.size - 1 or (psi_hat[idx - 1] > psi_hat[idx]
                                       and psi_hat[idx] < psi_hat[idx + 1]):
            break
...
def battle_lemarie_phi(N, Q, xi_min):
    if Q != 1:
        raise NotImplementedError(
            "Scaling BL wavelets to multiple wavelets per octave not implemented yet."
        )
...
def meyer_psi(N, Q, xi):
    if Q != 1:
        raise NotImplementedError(
            "Scaling Meyer wavelets to multiple wavelets per octave not implemented yet."
        )
...
def meyer_mother_psi(w):
    ...
    psi[idx] = np.sin(
        np.pi / 2 * nu(3 * np.abs(w[idx]) / 2 / np.pi - 1)) / np.sqrt(
            2 * np.pi)  # * np.exp(1j*w[idx]/2)

    idx = np.logical_and(4 * np.pi / 3 < w, w < 8 * np.pi / 3)
    psi[idx] = np.cos(
        np.pi / 2 * nu(3 * np.abs(w[idx]) / 4 / np.pi - 1)) / np.sqrt(
            2 * np.pi)  # * np.exp(1j*w[idx]/2)
